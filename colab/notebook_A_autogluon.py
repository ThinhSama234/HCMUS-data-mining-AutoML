...
# competitions (no --unzip flag — unzip manually below)
# NOTE: you must accept competition rules on kaggle.com first
...
!python scripts/orchestrator.py \
    --frameworks autogluon \
    --time-budget 60 \
    --no-venv \
    --run-id run_autogluon

print("Done.")


# CELL 7 — Download result (or mount Drive to save)
# -----------------------------------------------------------------------------
# Option A: download directly
from google.colab import files
import glob
for f in glob.glob("reports/run_autogluon*.json"):
    files.download(f)

# Option B: save to Google Drive
# from google.colab import drive
# drive.mount('/content/drive')
# !cp reports/run_autogluon*.json /content/drive/MyDrive/automl_reports/

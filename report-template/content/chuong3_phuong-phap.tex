...
\begin{figure}[H]
    \centering
    % Sơ đồ dùng TOẠ ĐỘ TUYỆT ĐỐI (at (x,y)) thay vì định vị tương đối
    % (below=of ...). Nhờ vậy hình vẫn vẽ đúng ngay cả khi thư viện
    % tikz "positioning" chưa được nạp trong preamble.
    \begin{tikzpicture}[
        x=1cm, y=1cm,
        every node/.style={font=\small},
        stage/.style={draw, rounded corners=2pt, align=center,
                      minimum height=0.95cm, minimum width=3.9cm,
                      inner sep=4pt, fill=blue!5},
        store/.style={draw, rounded corners=2pt, align=center,
                      minimum height=0.95cm, minimum width=2.4cm,
                      inner sep=4pt, fill=gray!12},
        wk/.style={draw, rounded corners=2pt, align=center,
                   minimum height=0.85cm, minimum width=2.5cm,
                   inner sep=3pt, fill=orange!12},
        arr/.style={->, >=stealth, thick}]

      \node[stage] (raw)   at (0,  0)    {Dữ liệu thô Kaggle};
      \node[stage] (prep)  at (0, -1.75) {Tiền xử lý\\\texttt{prep.py}};
      \node[store] (meta)  at (4.3,-1.75){\texttt{train.csv}\\\texttt{meta.json}};
      \node[stage] (fold)  at (0, -3.50) {Sinh phân hoạch fold\\\texttt{gen\_folds.py}};
      \node[stage] (orch)  at (0, -5.25) {Điều phối\\\texttt{orchestrator.py}};

      \node[wk] (w1) at (-3.2, -7.30) {Worker\\\texttt{flaml}};
      \node[wk] (w2) at ( 0.0, -7.30) {Worker\\\texttt{h2o}};
      \node[wk] (w3) at ( 3.2, -7.30) {Worker\\\texttt{autogluon}, \ldots};

      \node[stage] (merge) at (0, -9.35)  {Hợp nhất kết quả\\\texttt{merge.py}};
      \node[stage] (ana)   at (0, -11.10) {Phân tích và trực quan hoá};

      \draw[arr] (raw)   -- (prep);
      \draw[arr] (prep)  -- (meta);
      \draw[arr] (prep)  -- (fold);
      \draw[arr] (fold)  -- (orch);
      \draw[arr] (orch.south) -- (w1.north);
      \draw[arr] (orch.south) -- (w2.north);
      \draw[arr] (orch.south) -- (w3.north);
      \draw[arr] (w1.south) -- (merge.north);
      \draw[arr] (w2.south) -- (merge.north);
      \draw[arr] (w3.south) -- (merge.north);
      \draw[arr] (merge) -- (ana);
    \end{tikzpicture}
